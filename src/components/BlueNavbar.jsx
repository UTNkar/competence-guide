...
const BlueNavbar = () => {
  ...
  // This variable switches values when user presses the button in the
  // BlueNavbar, showing
  const [renderSectionsOrCommittees, setRenderSectionsOrCommittees] =
    useState(false);

  // This function is activated when the Sections/Committees button is pressed, changes the "renderSectionsOrCommittees value"
  const handleChangeComittesSections = () => {
    setRenderSectionsOrCommittees(!renderSectionsOrCommittees);
  };
  ...
  return (
    <>
        ...
        >
          ...
          >
            <div>
              {/* This code checks if the user has clicked the Sections/Committees button,
                  if renderSectionsOrCommittees = false it uses the first 4 elements in 
                  the array imagelinks, which contains info for Committees */}
              {renderSectionsOrCommittees
                ? imagelinks
                    .slice(0, 4)
            ...
            <div>
              ...
              >
                <CompareArrowsIcon />
                {/* The following code makes it so that the button text changes 
                    when it's clicked */}
                {renderSectionsOrCommittees ? (
